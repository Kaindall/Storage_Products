...
public class ProductFacade {
    ...
    public void increment(Long id, int quantity) {
        Product product = productService.add(id, quantity);
        registryService.add(product, quantity);
    }

    public void decrease(Long id, int quantity) {
        Product product = productService.decrease(id, quantity);
        registryService.decrease(product, quantity);
    }

    public void remove(Long id) {
        Product product = productService.find(id);
        productService.delete(id);
        registryService.decrease(product, product.quantity());
    }
